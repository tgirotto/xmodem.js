 ...
 * wrapper for .on
 */
Xmodem.prototype.on = function(eventName, callback) {
...
Xmodem.prototype.send = function(socket, dataBuffer) {
   ...
   */
  _self.eventEmitter.emit('ready', packagedBuffer.length - 1); // We don't count the filler
  ...
  const sendData = function(data) {
    ...
    if(data[0] === CRC_MODE && blockNumber === _self.XMODEM_START_BLOCK) {
      ...
      if(packagedBuffer.length > blockNumber) {
         ...
         */
        _self.eventEmitter.emit('start', _self.XMODEM_OP_MODE);
        sendBlock(socket, blockNumber, packagedBuffer[blockNumber], _self.XMODEM_OP_MODE);
        _self.eventEmitter.emit('status', { action: 'send', signal: 'SOH', block: blockNumber });
        blockNumber++;
      }
    }
     ...
     */
      ...
      _self.eventEmitter.emit('status', { action: 'recv', signal: 'ACK' });
      if(packagedBuffer.length > blockNumber) {
        sendBlock(socket, blockNumber, packagedBuffer[blockNumber], _self.XMODEM_OP_MODE);
        _self.eventEmitter.emit('status', { action: 'send', signal: 'SOH', block: blockNumber });
        blockNumber++;
        ...
        if(sent_eof === false) {
          ...
          _self.eventEmitter.emit('status', { action: 'send', signal: 'EOT' });
          socket.write(new Buffer([EOT]));
        }
        else {
          // We are finished!
          log.info('[SEND] - Finished!');
          _self.eventEmitter.emit('stop', 0);
      ...
      if (blockNumber === packagedBuffer.length && sent_eof) {
        ...
        _self.eventEmitter.emit('status', { action: 'recv', signal: 'NAK' });
        blockNumber--;
        if(packagedBuffer.length > blockNumber) {
          sendBlock(socket, blockNumber, packagedBuffer[blockNumber], _self.XMODEM_OP_MODE);
          _self.eventEmitter.emit('status', { action: 'send', signal: 'SOH', block: blockNumber });
